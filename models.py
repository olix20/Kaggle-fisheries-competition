...
class Vgg16BN():
    ...
    def fit_val(self, trn_path, val_path, nb_trn_samples, nb_val_samples, nb_epoch=1, callbacks=[], aug=False,weights=None):
        ...
        print ("training class indices: ",trn_gen.class_indices)
        print ("validation class indices: ",val_gen.class_indices)
        print ("trn_gen.nb_sample: ",trn_gen.nb_sample)
        print ("val_gen.nb_sample: ",val_gen.nb_sample)

        if weights is None:
            print ('no weights')
            self.model.fit_generator(trn_gen, samples_per_epoch=trn_gen.nb_sample, nb_epoch=nb_epoch,
                                 validation_data=val_gen, nb_val_samples=val_gen.nb_sample, callbacks=callbacks)
        else:
            print ('with weights: ',weights)
            self.model.fit_generator(trn_gen, samples_per_epoch=trn_gen.nb_sample, nb_epoch=nb_epoch,
                             validation_data=val_gen, nb_val_samples=val_gen.nb_sample, callbacks=callbacks,class_weight=weights)
